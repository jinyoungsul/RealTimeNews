					...
					$.ajax({
						...
						success : function(data) {
							$(data).find('keyword').each(
									function(index) {
										var ranking = $(this).find('ranking')
												.text();
										var searchkey = $(this).find(
												'searchkey').text();
										var i = index + 1;
										$(
												'table#naverrank tr:eq(' + i
														+ ') td:eq(' + (i - i)
														+ ')').text(ranking);
										$(
												'table#naverrank tr:eq(' + i
									...
									if (data != 'empty') {
										$(data).find('news').each(function() {
											...
											if (id == loginId&& state == 'true') {
												...
												result += "<button type='button' class='moreCo btn btn-info commentBtn' id='5' value="+newsNo+">더보기</button>";
												result += "</div>";
				...
				$(document)
						.on(
								...
								function() {
									...
									$(this)
											.toggleClass(
													"btn btn-default btn-lg btn btn-default btn-lg yellow");
									$.ajax({
										url : "news.do",
										type : "post",
										data : {
	...
	<div class="container-fluid">
		<div class="row">
			...
			<div class="col-sm-8 col-sm-offset-4 col-md-10 col-md-offset-2 main">
				...
				<h2 class="sub-header">이 시간 핫한 뉴스!</h2>
